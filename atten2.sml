let
  ...
  *)
  ...
  fun stage (inLoad, dB) = let
    ...
    val rSeries = 
          if balanced
          then 2.0 * round_e96 (rSeriesIdeal / 2.0)
          else round_e96 rSeriesIdeal
   ...
   * open or closed.
   *)
  fun foldStage ((stage, true), (load, atten)) = (outOpen load stage, atten * (attenOpen load stage))
    | foldStage ((stage, false), (load, atten)) = (load, atten)
